...
从 [GitHub Releases](https://github.com/TetraploidHuman/Action/releases) 下载对应平台的预编译包：

| 平台 | 包名 |
|------|------|
| Linux x64 | `action-*-linux-x64.tar.gz` |
| Linux ARM64 | `action-*-linux-arm64.tar.gz` |
...
# 编译为可执行文件
action build file.at -o output

# 类型检查（不运行）
action run file.at --check

# 优化编译
action run file.at -O 3

# 指定目标平台
action run file.at --target wasm

# 输出 IR/汇编
action build file.at --emit ir
action build file.at --emit asm

# 创建新项目
action init my_project
```
