...
| 平台 | 包名 |
|------|------|
| Linux x64 | `atomic-*-linux-x64.tar.gz` |
| Linux ARM64 | `atomic-*-linux-arm64.tar.gz` |
| Windows x64 | `atomic-*-windows-x64.zip` |

```bash
# Linux 安装示例
tar xzf atomic-0.1.0-linux-x64.tar.gz
export PATH="$PWD/atomic-0.1.0-linux-x64/bin:$PATH"
atomic run hello.at
```
