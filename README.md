...
[![CI](https://github.com/TetraploidHuman/atomic-lang/actions/workflows/ci.yml/badge.svg)](https://github.com/TetraploidHuman/atomic-lang/actions/workflows/ci.yml)
...
从 [GitHub Releases](https://github.com/TetraploidHuman/atomic-lang/releases) 下载对应平台的预编译包：

| 平台 | 静态链接 | 动态链接 |
|------|---------|---------|
| Linux x64 | `atomic-*-linux-x64-static.tar.gz` | `atomic-*-linux-x64-dynamic.tar.gz` |
| Linux ARM64 | `atomic-*-linux-arm64-static.tar.gz` | `atomic-*-linux-arm64-dynamic.tar.gz` |
| Windows x64 | `atomic-*-windows-x64-static.zip` | `atomic-*-windows-x64-dynamic.zip` |

- **静态链接**：LLVM 编译进二进制，单文件即可运行，体积较大（~150MB）
- **动态链接**：二进制较小，需保留 `lib/` 目录中的 LLVM 共享库

```bash
# Linux 安装示例
tar xzf atomic-0.1.0-linux-x64-static.tar.gz
export PATH="$PWD/atomic-0.1.0-linux-x64-static/bin:$PATH"
atomic run hello.at
```
